...
class CLIP(nn.Module):
    ...
    def get_text_encoder(self, text):
        text_features = self.encode_text(text)
        text_features = text_features / text_features.norm(dim=1, keepdim=True)
        return text_features

    def get_image_encoder(self, image):
    ...
    def forward(self, image, text, return_loss=True):
        image = self.preprocess(image).to(self.device)
        image_features = self.get_image_encoder(image)
        text_features = self.get_text_encoder(text)

        loss_itc = self.get_contrastive_loss(image_features, text_features, idx=idx)
        return loss_itc
