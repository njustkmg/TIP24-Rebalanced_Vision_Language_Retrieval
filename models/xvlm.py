...
def build_vision_encoder(config, load_params=False):
    ...
    if config['use_clip_vit']:  # good performance, but only base model available
        ...
        vision_width = vision_config['vision_width']  # 1024

        vision_encoder = SwinTransformer(img_size=vision_config['image_res'],
                                         patch_size=4,
                                         in_chans=3,
                                         embed_dim=vision_config['embed_dim'],  # 128
                                         depths=vision_config['depths'],
                                         num_heads=vision_config['num_heads'],
                                         window_size=vision_config['window_size'],  # 12
                                         mlp_ratio=4.,
                                         qkv_bias=True,
                                         drop_rate=0.0,
                                         drop_path_rate=0.1,
    ...
    if config['use_swin'] and load_params:
...
def build_text_encoder(config, vision_width, load_text_params=False, use_mlm_loss=False, config_text=None):
    ...
    if use_mlm_loss:  # for pre-training, load_text_params by default (otherwise notimplemented)
        ...
        if config['use_roberta']:
            ...
            text_encoder = BertModel.from_pretrained(pretrained_model_name_or_path=config['text_encoder'],
                                                     config=config_text, add_pooling_layer=False)
...
class XVLMBase(nn.Module):
    def __init__(self, config=None, load_vision_params=False, load_text_params=False,
        ...
        if self.config['load_pretrain'] is True and self.config['auto'] == 1:
            self.lamba = nn.Parameter(torch.Tensor([0.5]))
        else:
            self.lamba = config['lamda']

        if self.config['distance'] == "wd":
            self.use_weight = True

            self.img_wd_proj = nn.Sequential(
                nn.Linear(self.vision_width, 1536),
                nn.GELU(),
                nn.Linear(1536, 768)
            )
            
            self.text_wd_proj = nn.Sequential(
                nn.Linear(self.text_width, 1536),
                nn.GELU(),
                nn.Linear(1536, 768)
            )
        ...
        if use_contrastive_loss:
            ...
            self.vision_proj = nn.Linear(self.vision_width, self.embed_dim)  # [1024, 256]
            ...
            if self.config['load_pretrain'] is True:
                self.sig_vision_proj = nn.Linear(self.vision_width, self.embed_dim)  # [1024, 256]
                self.sig_text_proj = nn.Linear(self.text_width, self.embed_dim)  # [768, 256]
                self.init_params.extend(['vision_proj.' + n for n, _ in self.sig_vision_proj.named_parameters()])
                self.init_params.extend(['text_proj.' + n for n, _ in self.sig_text_proj.named_parameters()])
        ...
        if use_matching_loss:
            ...
            if self.config['load_pretrain'] is True:
                if config['temp2'] == auto_init_temp:
                    self.temp2 = nn.Parameter(torch.ones([]) * config['temp2'])
                    self.init_params.extend(['temp2'])
                else:
                    self.temp2 = config['temp2']
    ...
    def get_features(self, image_embeds=None, text_embeds=None, single_image_embed=None, single_text_embed=None):
        ...
        elif single_image_embed is None:
            return F.normalize(self.vision_proj(image_embeds[:, 0, :]), dim=-1), \
                F.normalize(self.text_proj(text_embeds[:, 0, :]), dim=-1)
        else:
            return F.normalize(self.vision_proj(image_embeds[:, 0, :]), dim=-1), \
                F.normalize(self.text_proj(text_embeds[:, 0, :]), dim=-1), \
                F.normalize(self.sig_vision_proj(single_image_embed), dim=-1), \
                F.normalize(self.sig_text_proj(single_text_embed), dim=-1)


    def get_single_contrast(self, image_feat, text_feat, idx=None):
    ...
    def get_matching_loss(self, image_embeds, image_atts, image_feat, text_embeds, text_atts, text_feat, idx=None,
                          single_text=None, single_image=None):
        """
...
# ============================================================================================
        ...
        cross_pos = self.get_cross_embeds(image_embeds, image_atts, text_embeds=text_embeds, text_atts=text_atts)    # [2, 18, 768]
        cross_neg = self.get_cross_embeds(image_embeds_all, image_atts_all, text_embeds=text_embeds_all,
                                          text_atts=text_atts_all)                                                   # [4, 18, 768]
        output = self.itm_head(torch.cat([cross_pos[:, 0, :], cross_neg[:, 0, :]], dim=0))  # [6, 2]
        itm_labels = torch.cat([torch.ones(bs, dtype=torch.long),                           # [6]
                                torch.zeros(2 * bs, dtype=torch.long)], dim=0).to(image_embeds.device)

        if self.config['load_pretrain'] is False:
            return F.cross_entropy(output, itm_labels)

        if self.config['distance'] == "wd":
            # ================================================================================
            # ====================4: cal relation Wasserstein distance========================
            # ================================================================================
            text_feat_all = F.normalize(
                allgather(text_feat, torch.distributed.get_rank(), torch.distributed.get_world_size()))
            text_t2t = 1.0 - text_feat_all @ text_feat_all.t()
            
            single_text = self.text_wd_proj(single_text)
            single_image = self.img_wd_proj(single_image)
            
            single_text_all = F.normalize(
                allgather(single_text, torch.distributed.get_rank(), torch.distributed.get_world_size()))
            single_image_all = F.normalize(
                allgather(single_image, torch.distributed.get_rank(), torch.distributed.get_world_size()))
            fusion_text_all = F.normalize(
                allgather(cross_pos, torch.distributed.get_rank(), torch.distributed.get_world_size())[:, 0, :])
            
            single_text_t2t = single_text_all @ single_text_all.t() / self.temp2
            single_image_t2t = single_image_all @ single_image_all.t() / self.temp2
            v_values = fusion_text_all @ fusion_text_all.t() / self.temp2
            
            all_bs = single_text_t2t.size(0)
            mask = torch.eye(all_bs, dtype=bool)
            
            single_text_t2t[mask] = 0
            single_image_t2t[mask] = 0
            
            u_values = self.lamba * single_text_t2t + (1 - self.lamba) * single_image_t2t
            v_values[mask] = 0
            
            u_values = u_values * text_t2t
            v_values = v_values * text_t2t
            
            if self.use_weight:
                u_weights = text_t2t.clone()
                v_weights = text_t2t.clone()
            
            u_sorter = torch.argsort(u_values)
            v_sorter = torch.argsort(v_values)
            
            all_values = torch.cat((u_values, v_values), dim=-1)
            all_values = all_values.sort()[0]
            
            deltas = torch.diff(all_values)
            
            x, y = u_values.size(0), u_values.size(1)
            for r in range(x):
                u_sorter[r] += r * y
                v_sorter[r] += r * y
            
            u_cdf_indices = torch.searchsorted(torch.take(u_values, u_sorter), all_values[:, :-1].contiguous(), right=True)
            v_cdf_indices = torch.searchsorted(torch.take(v_values, v_sorter), all_values[:, :-1].contiguous(), right=True)
            
            if self.use_weight:
                u_sorted_cumweights = torch.cat(
                    (torch.zeros(u_weights.size(0), 1, device=u_values.device),
                    torch.cumsum(torch.take(u_weights, u_sorter), dim=-1)), dim=-1)
                v_sorted_cumweights = torch.cat(
                    (torch.zeros(v_weights.size(0), 1, device=u_values.device),
                    torch.cumsum(torch.take(v_weights, v_sorter), dim=-1)), dim=-1)
            
                x, y = u_sorted_cumweights.size(0), u_sorted_cumweights.size(1)
                for r in range(x):
                    u_cdf_indices[r] += r * y
                    v_cdf_indices[r] += r * y
            
                u_cdf = torch.take(u_sorted_cumweights, u_cdf_indices) / u_sorted_cumweights[:, -1].unsqueeze(-1)
                v_cdf = torch.take(v_sorted_cumweights, v_cdf_indices) / v_sorted_cumweights[:, -1].unsqueeze(-1)
            
            else:
                u_cdf = u_cdf_indices / u_values.size(0)
                v_cdf = v_cdf_indices / v_values.size(0)
            
            loss_kd = torch.sum(torch.multiply(torch.abs(u_cdf - v_cdf), deltas))/u_cdf.size(0)
            loss_kdi = 0
            loss_kdt = 0

            return F.cross_entropy(output, itm_labels), loss_kd, loss_kdi, loss_kdt

        elif self.config['distance'] == "kl":
            # ====================1: cal relation KD========================
            single_text_all = F.normalize(
                allgather(single_text, torch.distributed.get_rank(), torch.distributed.get_world_size()))
            single_image_all = F.normalize(
            ...
            single_text_t2t = single_text_all @ single_text_all.t() / self.temp2
            single_image_t2t = single_image_all @ single_image_all.t() / self.temp2
            fusion_text_t2t = fusion_text_all @ fusion_text_all.t() / self.temp2
            
            all_bs = single_text_t2t.size(0)
            mask = ~torch.eye(all_bs, dtype=bool)
            
            single_text_t2t = single_text_t2t[mask].flatten()
            single_image_t2t = single_image_t2t[mask].flatten()
            
            ensemble = self.lamba * single_text_t2t + (1 - self.lamba) * single_image_t2t
            fusion_text_t2t = fusion_text_t2t[mask].flatten()
            
            f_inv_ensemble = self.quantile_func(ensemble)
            g_inv_fusion_text_t2t = self.quantile_func(fusion_text_t2t)
            img_inv_single_image_t2t = self.quantile_func(single_image_t2t)
            txt_inv_single_text_t2t = self.quantile_func(single_text_t2t)
            # 计算距离
            loss_kd = torch.mean(torch.abs(f_inv_ensemble - g_inv_fusion_text_t2t))
            loss_kdi = torch.mean(torch.abs(img_inv_single_image_t2t - f_inv_ensemble))
            loss_kdt = torch.mean(torch.abs(txt_inv_single_text_t2t - f_inv_ensemble))
            return F.cross_entropy(output, itm_labels), loss_kd, loss_kdi, loss_kdt

        else:
            # ====================2: cal relation MAE\MSE========================
            single_image_all = F.normalize(
                allgather(single_image, torch.distributed.get_rank(), torch.distributed.get_world_size()))
            single_text_all = F.normalize(
                allgather(single_text, torch.distributed.get_rank(), torch.distributed.get_world_size()))
            ...
            # f_inv_ensemble = self.quantile_func(ensemble)
            # g_inv_fusion_text_t2t = self.quantile_func(fusion_text_t2t)
            if self.config['distance'] == "mae":
                loss_kd = torch.mean(torch.abs(fusion_text_t2t - ensemble))  # mae
                loss_kdi = torch.mean(torch.abs(single_image_t2t - ensemble))
                loss_kdt = torch.mean(torch.abs(single_text_t2t - ensemble))
            else:
                loss_kd = torch.mean((fusion_text_t2t - ensemble) ** 2)  # mse
                loss_kdi = torch.mean((single_image_t2t - ensemble) ** 2)
                loss_kdt = torch.mean((single_text_t2t - ensemble) ** 2)
            return F.cross_entropy(output, itm_labels), loss_kd, loss_kdi, loss_kdt



    def quantile_func(self, p):
